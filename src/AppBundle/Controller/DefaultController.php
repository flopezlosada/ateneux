 ...
 */
    ...
    public function warningAction($year = null)
    {
        ...
        foreach ($major_offence_type as $type) {
            $course_year_major_offence[] = array($em->getRepository("AppBundle:Warning")->findMajorOffenceByTypeYear($type, $year), $type->getTitle(), $type->getWarningType()); //valores por año
            foreach ($array_months as $month) {
        ...
        foreach ($penalty_type as $type) {
            $course_year_penalty[] = array($em->getRepository("AppBundle:Warning")->findPenaltyByTypeYear($type, $year), $type->getTitle(), $type->getWarningType()); //valores por año
            foreach ($array_months as $month) {
    ...
    public function course_change_indexAction()
    {
        ...
        if ($course_activation_status) {
            ...
            } else if ($course_activation_status->getStatus() == 2 ||$course_activation_status->getStatus() == 3) {
                return $this->redirect($this->generateUrl('dashboard'));
            }
        } else {
